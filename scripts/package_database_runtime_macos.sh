...
usage() {
    cat <<'EOF'
Usage:
  scripts/package_database_runtime_macos.sh --source prebuild/database/mysql-8.4.7 [options]

Convert a macOS database runtime into a self-contained distributable bundle by:
...
Options:
  --source <path>     Source database runtime directory. Required.
  --output <path>     Output directory. Default: ~/Library/Application Support/Server Engine/bin/database/<runtime-name>
...
if [ -z "$output_dir" ]; then
    ...
    output_dir="$HOME/Library/Application Support/Server Engine/bin/database/$(basename "$source_dir")"
fi
...
# Remove symlinks that point outside the runtime bundle (for example legacy
# init script links like bin/rcmysql -> /etc/init.d/mysql). These links break
# secure extraction in runtime manager and are not needed at runtime.
prune_unsafe_symlinks() {
    root="$1"
    /usr/bin/find "$root" -type l | while IFS= read -r link_path; do
        link_target="$(/bin/readlink "$link_path" || true)"
        [ -n "$link_target" ] || continue
        case "$link_target" in
            /*)
                printf 'Removing absolute symlink: %s -> %s\n' "$link_path" "$link_target"
                /bin/rm -f "$link_path"
                continue
                ;;
        esac
        link_parent="$(dirname "$link_path")"
        resolved_target="$(python3 - "$link_parent" "$link_target" <<'PY'
import os
import sys
parent, target = sys.argv[1], sys.argv[2]
print(os.path.realpath(os.path.join(parent, target)))
PY
)"
        case "$resolved_target" in
            "$root"/*) ;;
            *)
                printf 'Removing escaping symlink: %s -> %s\n' "$link_path" "$link_target"
                /bin/rm -f "$link_path"
                ;;
        esac
    done
...
prune_unsafe_symlinks "$output_dir"
...
printf 'Packaged database runtime:\n'
printf '  source: %s\n' "$source_dir"
printf '  output: %s\n' "$output_dir"
