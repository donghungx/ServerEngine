...
usage() {
    cat <<'EOF'
Usage:
  scripts/package_memcached_runtime_macos.sh --source prebuild/memcached/memcached-1.6.41 [options]

Convert a macOS Memcached runtime into a self-contained distributable bundle by:
...
Options:
  --source <path>     Source Memcached runtime directory. Required.
  --output <path>     Output directory. Default: ~/Library/Application Support/Server Engine/bin/memcached/<runtime-name>
...
if [ -z "$output_dir" ]; then
    ...
    output_dir="$HOME/Library/Application Support/Server Engine/bin/memcached/$(basename "$source_dir")"
fi
...
printf 'Packaged Memcached runtime:\n'
printf '  source: %s\n' "$source_dir"
printf '  output: %s\n' "$output_dir"
