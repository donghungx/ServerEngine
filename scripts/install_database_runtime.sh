...
usage() {
    cat <<'EOF'
Usage:
  scripts/install_database_runtime.sh
  scripts/install_database_runtime.sh --formula mariadb [options]

Install or stage a Homebrew database runtime into this repo's prebuild layout:
  prebuild/database/<engine>-<version>

Options:
  --formula <name>      Homebrew formula to install/stage.
                        Examples: mariadb, mysql
  --engine <name>       Override detected engine. Default: auto from formula
...
def load_options():
    ...
    def sort_key(item):
        name, _ = item
        brand_order = 0 if name.startswith("mariadb") else 1
        raw_version = name.split("@", 1)[1] if "@" in name else ""
        if raw_version:
            version_key = tuple(int(part) for part in raw_version.split("."))
        else:
            version_key = (999, 999)
        return (brand_order, tuple(-part for part in version_key))
...
def draw(fd, options, index):
    ...
    write(fd, "Select database runtime and press Enter\n\n")
...
if not options:
    raise SystemExit("No database runtime options found from Homebrew.")
...
installed_conflicting_formulas() {
    selected="$1"
    if printf '%s' "$selected" | grep -q '^mysql'; then
        brew list --formula 2>/dev/null | grep '^mariadb' || true
    else
        brew list --formula 2>/dev/null | grep '^mysql' || true
    fi
}
...
restore_homebrew_links() {
    ...
    if [ -n "$unlinked_formulas" ]; then
        ...
        IFS='
'
        for item in $unlinked_formulas; do
            [ -n "$item" ] || continue
            brew link --overwrite "$item" >/dev/null 2>&1 || true
        done
        IFS="$OLD_IFS"
    fi
}
...
if [ "$original_argc" -eq 0 ] && [ -z "$formula" ]; then
    ...
    if [ -z "$selected_formula" ]; then
        printf 'No database runtime selected.\n' >&2
        exit 1
    fi
...
if [ "$skip_install" != "1" ]; then
    conflicting_formulas="$(installed_conflicting_formulas "$formula")"
    if [ -n "$conflicting_formulas" ]; then
        OLD_IFS="$IFS"
        IFS='
'
        for item in $conflicting_formulas; do
            [ -n "$item" ] || continue
            if is_linked "$item"; then
                printf 'Temporarily unlinking conflicting Homebrew formula: %s\n' "$item"
                brew unlink "$item"
                if [ -z "$unlinked_formulas" ]; then
                    unlinked_formulas="$item"
                else
                    unlinked_formulas="$unlinked_formulas
$item"
                fi
            fi
        done
...
if [ "$engine" = "mariadb" ]; then
    if [ ! -x "$brew_prefix/bin/mariadbd" ]; then
        printf 'Expected MariaDB server binary missing: %s/bin/mariadbd\n' "$brew_prefix" >&2
        exit 1
    fi
else
    if [ ! -x "$brew_prefix/bin/mysqld" ]; then
        printf 'Expected MySQL server binary missing: %s/bin/mysqld\n' "$brew_prefix" >&2
        exit 1
    fi
fi
...
mkdir -p "$prefix/conf"
if [ "$engine" = "mariadb" ] && [ ! -f "$prefix/conf/my.cnf" ]; then
    if [ -f "$brew_prefix/support-files/my-medium.cnf" ]; then
        cp "$brew_prefix/support-files/my-medium.cnf" "$prefix/conf/my.cnf"
    else
        : > "$prefix/conf/my.cnf"
    fi
fi
...
if [ -x "$prefix/bin/mysql" ]; then
    printf '  %s\n' "$prefix/bin/mysql"
elif [ -x "$prefix/bin/mariadb" ]; then
    printf '  %s\n' "$prefix/bin/mariadb"
else
    printf '  not found\n'
fi
