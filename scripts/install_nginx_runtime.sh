...
usage() {
    cat <<'EOF'
Usage:
  scripts/install_nginx_runtime.sh
  scripts/install_nginx_runtime.sh --formula nginx [options]

Install or stage a Homebrew Nginx runtime into this repo's prebuild layout:
  prebuild/server/<engine>-<version>

Options:
  --formula <name>      Homebrew formula to install/stage.
                        Examples: nginx
  --engine <name>       Override detected engine. Default: nginx
...
def draw(fd, options, index):
    ...
    write(fd, "Select Nginx runtime and press Enter\n\n")
...
if not options:
    raise SystemExit("No Nginx runtime options found from Homebrew.")
...
if [ "$original_argc" -eq 0 ] && [ -z "$formula" ]; then
    ...
    if [ -z "$selected_formula" ]; then
        printf 'No Nginx runtime selected.\n' >&2
        exit 1
    fi
...
global_brew_prefix="$(brew --prefix 2>/dev/null || true)"
...
server_binary=""
if [ -x "$brew_prefix/bin/nginx" ]; then
    server_binary="$brew_prefix/bin/nginx"
elif [ -x "$brew_prefix/sbin/nginx" ]; then
    server_binary="$brew_prefix/sbin/nginx"
else
    printf 'Expected Nginx server binary missing under %s/bin or %s/sbin\n' "$brew_prefix" "$brew_prefix" >&2
    exit 1
fi
...
mkdir -p "$prefix/logs"
mkdir -p "$prefix/run"
if [ ! -f "$prefix/conf/nginx.conf" ]; then
    if [ -f "$brew_prefix/etc/nginx/nginx.conf" ]; then
        cp "$brew_prefix/etc/nginx/nginx.conf" "$prefix/conf/nginx.conf"
    elif [ -n "$global_brew_prefix" ] && [ -f "$global_brew_prefix/etc/nginx/nginx.conf" ]; then
        cp "$global_brew_prefix/etc/nginx/nginx.conf" "$prefix/conf/nginx.conf"
    else
        : > "$prefix/conf/nginx.conf"
    fi
fi
if [ ! -f "$prefix/conf/mime.types" ]; then
    if [ -f "$brew_prefix/conf/mime.types" ]; then
        cp "$brew_prefix/conf/mime.types" "$prefix/conf/mime.types"
    elif [ -f "$brew_prefix/.bottle/etc/nginx/mime.types" ]; then
        cp "$brew_prefix/.bottle/etc/nginx/mime.types" "$prefix/conf/mime.types"
    elif [ -n "$global_brew_prefix" ] && [ -f "$global_brew_prefix/etc/nginx/mime.types" ]; then
        cp "$global_brew_prefix/etc/nginx/mime.types" "$prefix/conf/mime.types"
    fi
fi
if [ ! -f "$prefix/conf/fastcgi_params" ]; then
    if [ -f "$brew_prefix/conf/fastcgi_params" ]; then
        cp "$brew_prefix/conf/fastcgi_params" "$prefix/conf/fastcgi_params"
    elif [ -f "$brew_prefix/.bottle/etc/nginx/fastcgi_params" ]; then
        cp "$brew_prefix/.bottle/etc/nginx/fastcgi_params" "$prefix/conf/fastcgi_params"
    elif [ -n "$global_brew_prefix" ] && [ -f "$global_brew_prefix/etc/nginx/fastcgi_params" ]; then
        cp "$global_brew_prefix/etc/nginx/fastcgi_params" "$prefix/conf/fastcgi_params"
    fi
fi
...
if [ "$skip_package" != "1" ]; then
    package_script="$repo_root/scripts/package_nginx_runtime_macos.sh"
