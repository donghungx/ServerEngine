...
usage() {
    cat <<'EOF'
Usage:
  scripts/package_redis_runtime_macos.sh --source prebuild/redis/redis-8.0.3 [options]

Convert a macOS Redis runtime into a self-contained distributable bundle by:
...
Options:
  --source <path>     Source Redis runtime directory. Required.
  --output <path>     Output directory. Default: ~/Library/Application Support/Server Engine/bin/redis/<runtime-name>
...
if [ -z "$output_dir" ]; then
    ...
    output_dir="$HOME/Library/Application Support/Server Engine/bin/redis/$(basename "$source_dir")"
fi
