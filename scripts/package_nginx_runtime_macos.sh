...
usage() {
    cat <<'EOF'
Usage:
  scripts/package_nginx_runtime_macos.sh --source prebuild/server/nginx-1.29.2 [options]

Convert a macOS Nginx runtime into a self-contained distributable bundle by:
...
Options:
  --source <path>     Source Nginx runtime directory. Required.
