...
usage() {
    cat <<'EOF'
Usage:
  scripts/install_redis_runtime.sh
  scripts/install_redis_runtime.sh --formula redis [options]

Install or stage a Homebrew Redis runtime into this repo's prebuild layout:
  prebuild/redis/redis-<version>

Options:
  --formula <name>      Homebrew formula to install/stage.
                        Examples: redis, redis-stack
  --engine <name>       Override detected engine. Default: redis
  ...
  --deploy-root <path>  Override deploy root. Default: <repo>/dist/runtime/redis
  --help                Show this help

Examples:
  scripts/install_redis_runtime.sh
  scripts/install_redis_runtime.sh --formula redis --overwrite
  scripts/install_redis_runtime.sh --formula redis-stack --overwrite
EOF
}
...
def draw(fd, options, index):
    ...
    write(fd, "Select Redis runtime and press Enter\n\n")
...
if not options:
    raise SystemExit("No Redis runtime options found from Homebrew.")
...
if [ "$original_argc" -eq 0 ] && [ -z "$formula" ]; then
    ...
    if [ -z "$selected_formula" ]; then
        printf 'No Redis runtime selected.\n' >&2
        exit 1
    fi
...
if [ -z "$prefix" ]; then
    prefix="$repo_root/prebuild/redis/$engine-$version"
fi
...
if [ ! -x "$brew_prefix/bin/redis-server" ]; then
    printf 'Expected Redis server binary missing: %s/bin/redis-server\n' "$brew_prefix" >&2
    exit 1
fi
...
mkdir -p "$prefix/data"
mkdir -p "$prefix/logs"
if [ ! -f "$prefix/conf/redis.conf" ]; then
    if [ -f "$brew_prefix/etc/redis.conf" ]; then
        cp "$brew_prefix/etc/redis.conf" "$prefix/conf/redis.conf"
    elif [ -f "$brew_prefix/etc/redis/redis.conf" ]; then
        cp "$brew_prefix/etc/redis/redis.conf" "$prefix/conf/redis.conf"
    else
        : > "$prefix/conf/redis.conf"
    fi
fi
...
printf 'CLI binary:\n'
if [ -x "$prefix/bin/redis-cli" ]; then
...
if [ "$skip_package" != "1" ]; then
    package_script="$repo_root/scripts/package_redis_runtime_macos.sh"
    if [ -f "$package_script" ]; then
        ...
        if [ -n "$package_output" ]; then
            ...
            packaged_dir="$repo_root/dist/runtime/redis/$(basename "$prefix")"
            /bin/sh "$package_script" --source "$prefix" --overwrite --output "$packaged_dir"
        fi
    else
...
if [ "$skip_package" != "1" ] && [ "$skip_deploy" != "1" ]; then
    if [ -z "$deploy_root" ]; then
        deploy_root="$repo_root/dist/runtime/redis"
    fi
